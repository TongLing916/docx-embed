...
def demo_xlsx_path() -> Path:
    """Path to the pre-built demo workbook fixture.

    Generated by ``tests/generate_demo_xlsx.py``. Contains:
    - Sales Summary (formulas, chart, merged cells, hyperlink, comment)
    - Inventory (hidden row + column, date values, stock-value formulas)
    - _Config (hidden sheet)
    - Data Types (text, int, float, bool, date, datetime, percentage, currency)
    """

    return Path(__file__).parent / "fixtures" / "demo_workbook.xlsx"
